...
public class GraphGUI implements IGUI {
	...
	private JCheckBox sensorSimul = new JCheckBox("Simulate external sensor",
			false);
	 ...
	 * 
	 * @param robot
	 *            The robot to link to this GUI
	 * @param robotGUI
	 *            Reference to the robotGUI
	 */
	 ...
	 * 
	 *          ActionListener from the GraphGUI. Listens to the generated
	 *          events and handles accordingly.
	 */
	private class RobotGUIActionListener implements ActionListener {
		 ...
		 * 
		 * @param arg0
		 *            ActionEvent that generated by the window
		 */
		public void actionPerformed(ActionEvent arg0) {
			// if RobotGUI is closed, then the grapGUI also needs to be closed
			if (!robotGUI.isVisible() && !robotGUI.isVisibleInLeftFrame())
			...
			if (arg0.getSource() == meting_running) {
				if (measuring) {
					...
					theServer = new SensorServer(isSensorSimulated); // Start
																		// server
					measuring = true;
	...
	/*
	 * Functie die een meting opent uit een ini-bestand afkomstig van de harde
	 * schijf
	 */
	private void openMeting() {
		...
		try {
			...
			if (!line.equals("[Meting]")) {
				...
				JOptionPane.showMessageDialog(null, "Invalid file formaat",
			...
			if (samples != 128) {
				...
				JOptionPane.showMessageDialog(null,
						"Samplesize doesn't match 128", "Open file failed",
			...
			for (eindindex = index; eindindex < line.length(); eindindex++) {
				...
				if (line.charAt(eindindex) == ',') {
					...
					// zetten
					meting[metingnr] = Byte.parseByte(line.substring(index,
							eindindex));
					index = eindindex + 1;
					metingnr++;
				}
			...
			for (eindindex = index; eindindex < line.length(); eindindex++) {
				if (line.charAt(eindindex) == ',') {
					metingFFT[metingnr] = Byte.parseByte(line.substring(index,
							eindindex));
					index = eindindex + 1;
					metingnr++;
				}
	...
	private void updateGraphData() {
		...
		/*
		 * Change: We not read the result into a special variable, so that it
		 * can be passed along to our own module for speech recognition. After
		 * that, it is plotted to the screen as normal.
		 */
	...
	private void updateSpraak() {
		...
		if (piekenVer[0] < STORING) {
			...
			// Nothing
		} else if (((piekenDicht[0] - piekenDicht[1]) > WILLEKEUR)
				&& (piekenDicht[1] > STORING)) {
			// Nothing
		} else if (((piekenVer[0] - piekenVer[1]) > WILLEKEUR)
				&& (piekenVer[1] > STORING_VER)) {
			// Nothing
			...
			recStatus[0].setBackground(Color.GREEN);
				...
				&& (speechData[piekenDicht[0]] > STORING)
				&& (piekenVer[0] >= 25) && (piekenVer[0] <= 35)
				&& (speechData[piekenVer[0]] > STORING_VER)) {
			recStatus[1].setBackground(Color.GREEN);
		} else if ((piekenDicht[0] >= 2) && (piekenDicht[0] <= 4)
				&& (speechData[piekenDicht[0]] > STORING)
				&& (piekenVer[0] >= 30) && (piekenVer[0] <= 45)
				&& (speechData[piekenVer[0]] > STORING_VER)) {
	 ...
	 * 
	 * @param visible
	 *            boolean if visible (true) or not (false)
	 */
	 ...
	 * 
	 * @return boolean - true if the checkbox is set (sensorvalues will be
